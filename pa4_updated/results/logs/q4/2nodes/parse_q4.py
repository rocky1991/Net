with open('local_produce_2nodes.log', 'U') as file:
	l = file.read()
	# print(line)
	while 'Produce message:(key=' in l:
		l=l.replace('Produce message:(key=','')
	while ') to topic: topic2 at time:' in l:
		l=l.replace(') to topic: topic2 at time:',',')
	while ' msg_size=' in l:
...
with open('responder1_topic2_4.log','U') as file:
	line = file.read()
...
# 		file.write(new_line)
with open('responder2_topic2_4.log','U') as file:
	line = file.read()
	while "Receive message: (key=b'" in line:
		line=line.replace("Receive message: (key=b'",'')
	while ') from topic: topic2 at time:' in line:
