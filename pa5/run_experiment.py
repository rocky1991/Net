		...
		if q_num == '3':
			...
			for msg in msg_list:
				for i in range(20):
					...
					ts = str(time.time())
					append_to_file('q3_initiator.log',"Insert Doc:(counter:{},msg_size={},time: {})\n".format(counter,len(msg),ts))
					counter+=1
			...
			for nodes in nodes_setting:
				...
				append_to_file('q4_initiator.log','Num of nodes:{}\n'.format(nodes))
				...
				for msg in msg_list:
					...
					client.pa5.messages1.insert_one({'counter':counter,'value':msg})
					ts = str(time.time())
					append_to_file('q4_initiator.log',"Insert Doc:(counter:{},msg_size={},time: {})\n".format(counter,len(msg),ts))
					counter+=1
			# print("Deleting all documents")
			# client.pa5.messages1.delete_many({})

		elif q_num == '5':
			write_to_file('q5_initiator.log','')
			...
			for loss_setting in loss_settings:
				...
				append_to_file('q5_initiator.log','loss setting = {}\n'.format(loss_setting))
				...
				for msg in msg_list:
					...
					client.pa5.messages1.insert_one({'counter':counter,'value':msg})
					ts = str(time.time())
					append_to_file('q5_initiator.log',"Insert Doc:(counter:{},msg_size={},time: {})\n".format(counter,len(msg),ts))
					counter+=1
			# print("Deleting all documents")
			# client.pa5.messages1.delete_many({})
		elif q_num == '6':
			write_to_file('q6_initiator.log','')
			print("Number of responder nodes: {}".format(str(3)))
			append_to_file('q6_initiator.log','Num of nodes:{}\n'.format(str(3)))
			input('Please log in each responder and start pulling messages\n Run "python3 pull_msg.py q6_responder_num.log messages1 False None"')
			msg_list = ['a'*32]*50
			loss_settings = [0.2 * c for c in range(0,11)]
			counter = 0
			for loss_setting in loss_settings:
				...
				append_to_file('q6_initiator.log','loss setting:{}\n'.format(loss_setting))
				print("Adjusting network setting")
				for i,instance in enumerate(get_ip()):
					# if(i>4):
					# 	break
				...
				for msg in msg_list:
					...
					client.pa5.messages1.insert_one({'counter':counter,'value':msg})
					ts = str(time.time())
					append_to_file('q6_initiator.log',"Insert Doc:(counter:{},msg_size={},time: {})\n".format(counter,len(msg),ts))
					counter+=1
			...
			append_to_file('q9_initiator.log',"Q9 >>>>>>>>>>>>>>>>\n")
			msg_list = ['a' * 2**c for c in range(0,21)]
			counter = 0
			for msg in msg_list:
				...
				client.pa5.messages1.insert_one({'counter':counter,'value':msg})
				ts = str(time.time())
				append_to_file('q9_initiator.log',"Insert Doc:(counter:{},msg_size={},time: {})\n".format(counter,len(msg),ts))
				counter+=1
		elif q_num == '10':
			write_to_file('q10_initiator.log','')
			...
			append_to_file('q10_initiator.log',"Q{} >>>>>>>>>>>>>>>>\n".format(q_num))
			msg_list = ['a'*32] * 50
			print("Adjusting network setting")
			input('Please adjust network setting on each node\nSet latency 15, bandwidth 10m, loss 0.0000000001')
			for i,instance in enumerate(get_ip()):
			...
			for delay in delays:
				...
				append_to_file('q10_initiator.log','delay setting:{}\n'.format(delay))
				for msg in msg_list:

					print("current message id is {} size is {}".format(counter,len(msg)))
					client.pa5.messages1.insert_one({'counter':counter,'value':msg})
					ts = str(time.time())
					append_to_file('q10_initiator.log',"Insert Doc:(counter:{},msg_size={},time: {})\n".format(counter,len(msg),ts))
					counter+=1
					time.sleep(delay)
			# print("Deleting all documents")
			# client.pa5.messages1.delete_many({})
